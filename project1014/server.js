...
var mysql = require("mysql"); //mysql외부 모듈
...
var server = http.createServer(function(request,response){
    ...
    //클라이언트가 원하는 요청을 처리하려면, URL분석을 하여
    //URI추출해서 조건을 따져보자
    var urlJson = url.parse(request.url,true);//분석 결과를 json으로 반환
    ...
    if(urlJson.pathname=="/"){
        ...
        con.query(sql,function(error,record,fields){
            ...
            var tag ="<table width='100%' border='2px'>";
                ...
                for(var i=0;i<record.length;i++){
                    var member = record[i]; //각각의 json을 끄집어내자
                    tag+="<tr>";
        ...
        con.query(sql, function(error,record,fields){ 
            if(error){
                ...
                fs.readFile("./detail.ejs","utf-8",function(error,data){
                    if(error){
                        console.log("detail.html읽기 실패",error);
                    }else{
                        response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
                        response.end(ejs.render(data,{  //그냥 보내지말고, 서버에서 실행한 후 그 결과를 보내자
                            member:obj   //json자체를 보내버림
                        }));
                    }
                });
        ...
        fs.readFile("./test.ejs","utf-8",function(error,data){
            response.writeHead(200,{"Content-Type":"text/html;charset=utf-8"});
            //ejs를 그냥 파일로 문자취급해서 보내면 원본코드까지 가버리기 떄문에,,
            //서버에서 실행을 한 후, 그결과를 보내야 한다..(jsp,php,asp의 원리)
...
server.listen(7878,function(){
    console.log("My server is running at 7878port....");
    getConnection();
});
