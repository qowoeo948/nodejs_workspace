...
var mysql = require("mysql"); //외부모듈이기 떄문에 npm install로 설치해야함
    ...
    if(result.pathname=="/login"){
        ...
        con.query(sql,function(error,record,fields){
            if(error){
                ...
                if(record.length>0){
                    //레코드가 있을때는(배열의 길이가 1), 로그인 성공 메세지
                    // console.log("인증 성공!!!");
                    response.end("<script>alert('인증 성공!!');</script>"); //클라이언트 브라우저에 출력
                }else{
                    //레코드가 없을때는(배열의 길이가 0), 로그인 실패 메세지
                    // console.log("인증 실패!!!");
        ...
        fs.readFile("./2_loginForm.html", "utf-8", function(error, data){
            if(error){
