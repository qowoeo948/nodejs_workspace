...
var mysql = require("mysql"); //외부모듈, 다운한거 pnm
var fs = require("fs"); //파일시스템 내장모듈

var con; 
//접속 성공시 그 정보를 보유한 객체, 이객체가 있어야 접속된 상태에서 쿼리문을
//수행할수 있음
...
var server = http.createServer(function(request,response){
    ...
    //클라이언트가 원하는 것이 무언인지를 구분하기 위한 url분석!!
    console.log("클라이언트의 요청 url : ",request.url);//한줄로되어있어 구분을 못함
    //섞여있는 url을 분석(parsing)하기 위해서는 전담 모듈을 이용하자!!
    //url 내장모듈 -> var url = require("url");
   var parseObj = url.parse(request.url,true);  //분석시작~~
   //분석시 true를 매개변수로 전달하면, 파마리터들을 {json}으로 묶어준다.
   //장점? json은 객체표기법이므로, 데이터들을 .점찍고 마치 객체 다루듯 접근할수
   //있기 떄문에 직관성이 높아진다. 즉 다루기 쉬워진다. (배열보다 낫다^^)
    ...
    if(parseObj.pathname=="/member/registForm"){//회원가입 양식디자인 폼을 원하면
        fs.readFile("./2_registForm.html","utf-8",function(error,data){
            if(error){
                console.log("읽기 실패",error);
            }else{
                response.end(data); //html파일을 읽어들인 결과문자열을 클라이언트에 
                                                //응답정보로 저장
            }
        });
        ...
        con.query(sql,
            ...
            param.memo], function(error,result,fields){
                if(error){
                    ...
                    var msg="<script>";
                    msg+="alert('가입성공');"
                    msg+="location.href='/member/list';" //<a herf="">동일
                    //member/list로 재접속(클라이언트가 지정한 주소로 재접속함)
                    msg+="</script>";
